    ...
    for (int j = 0; j < m_res_y; ++j) {
        for (int i = 0; i < m_res_x; ++i) {
            ...
            for (size_t k = 0; k < vertices.size(); ++k) {
                const Vec3& v1 = vertices[k];
                const Vec3& v2 = vertices[(k + 1) % vertices.size()];
            ...
            // 根据点是否在多边形内部设置符号
            m_field_values[idx] = inside ? -min_dist : min_dist;
        }
    }
}
...
// 新增：从txt文件读取点集
inline std::vector<Vec3> load_points_from_txt(const std::string& filename) {
